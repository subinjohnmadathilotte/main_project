...
$date=date("Y-m-d");
 ...
 {  
			...
			$res_id=mysqli_query($con,"select max(not_id)  from notification");
            $row_ld=mysqli_fetch_array($res_id) ;	
			$id=$row_ld["max(not_id)"];
			 $new_id=$id+1;
		$sub=$_POST["subject"];
...
//checking checkbox 
	 foreach($_POST['cb_list'] as $item)
	 {
		...
		$sql="insert into notification (`not_id`,`subject`,`content`, `district`, `date_limit`, `date`)
		values('$new_id','$sub','$content',$item,'$date_lim','$date')";
		...
		<a href="#" data-toggle="collapse" data-target="#toggleDemo" data-parent="#sidenav01" class="collapsed">
          ...
          <div class="collapse" id="toggleDemo" style="height: 0px;">
            <ul class="nav nav-list">
              <li class="active1"><a href="">Add notification</a></li>
...
<form action="" method="POST" name="my_form">
		...
		<div class="col-md-4 offset-1">
			<input  type="text"  name="subject" id="subject" class=" form-control"   onchange="sub();" required /> 
		</div>
		</div>
		...
		?>
			 <input  type="date"  name="date_lim" max="2020-01-01" min="<?php echo $date ?>" class="form-control" required/>
		</div>
		</div>
  ...
  <script>
	 ...
	 var val_subject= /^[A-Za-z]+$/;
	 $subject= document.getElementById('subject').value;
	
	 if(!val_subject.test($subject)){
